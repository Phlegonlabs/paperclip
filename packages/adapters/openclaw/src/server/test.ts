  ...
  if (mode === "authenticated" && exposure === "private") {
    if (bindHost && !isLoopbackHost(bindHost) && !allowSet.has(bindHost)) {
      checks.push({
        ...
        hint: `Run bun run paperclipai -- allowed-hostname ${bindHost} so remote OpenClaw callbacks can pass host checks.`,
      });
    }
    ...
    if (endpointHost && !isLoopbackHost(endpointHost) && allowSet.size === 0) {
      checks.push({
        ...
        hint: "Set one with bun run paperclipai -- allowed-hostname <host> when OpenClaw runs on another machine.",
      });
    }
  }
