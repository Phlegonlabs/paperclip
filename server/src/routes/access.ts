  ...
  if (input.deploymentMode === "authenticated" && input.deploymentExposure === "private") {
    ...
    if (bindHost && !isLoopbackHost(bindHost) && !allowSet.has(bindHost)) {
      diagnostics.push({
        ...
        hint: `Run bun run paperclipai -- allowed-hostname ${bindHost}`,
      });
    }
    if (callbackHost && !isLoopbackHost(callbackHost) && allowSet.size === 0) {
      diagnostics.push({
        ...
        hint: "Set one with bun run paperclipai -- allowed-hostname <host> when OpenClaw runs off-host.",
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run bun run paperclipai -- allowed-hostname ${apiHost}`,
    });
  }
      ...
      connectivity: {
        ...
        guidance:
          opts.deploymentMode === "authenticated" && opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Paperclip hostname is reachable and allowed via `bun run paperclipai -- allowed-hostname <host>`."
